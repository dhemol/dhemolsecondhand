...
if (!empty($_FILES["gbarang"]["tmp_name"]))
{
    ...
    {           
        ...
        if (move_uploaded_file($_FILES['gbarang']['tmp_name'], $gambar)) 
		{
			...
			$query = mysqli_query($con,"UPDATE barang SET nama_brg='$namaBarang', harga_brg='$hrgBarang', desc_brg='$descBarang', jml_brg='$jmlBrg', kategori='$kategoriBrg', gambar_brg='$gambar_barang' where id_brg='$kdBrg'") or die(mysqli_error());
	
            if ($query) 
			{
				header('location:produk.php?message=update');
			}
        } 
		else 
		{
